...
class AnalysisHandler:
    ...
    async def handle_analysis_request(self, request_data: Dict) -> Dict[str, Any]:
        """Handle analysis request with store_alerts flag"""
        try:
            ...
            logger.info(f"Analysis params: {network} {analysis_type}, {num_wallets} wallets, {days_back} days, store_data={store_data}, store_alerts={store_alerts}")
            ...
            debug_info = self._build_debug_info(
                ...
                use_smart_timing, max_tokens, min_alpha_score, telegram_status, store_data, store_alerts
            )
            ...
            try:
                ...
                # Build result
                result_dict = self._build_success_result(result, days_back, use_smart_timing, debug_info, store_data, store_alerts)
                
                storage_msg = f", stored {result.performance_metrics.get('transfers_stored', 0)} transfers" if store_data else ", no transfer storage"
                alerts_msg = f", alerts stored" if store_alerts else ", no alert storage"
                
                logger.info(f"Enhanced analysis complete - {result.total_transactions} transactions, {result.unique_tokens} tokens{storage_msg}{alerts_msg}")
    ...
    def _build_debug_info(self, network: str, analysis_type: str, num_wallets: int,
                     ...
                     telegram_status: Dict, store_data: bool, store_alerts: bool) -> Dict:
        """Build debug information with store_alerts flag"""
    ...
    def _build_success_result(self, result, days_back: float, use_smart_timing: bool, 
                         debug_info: Dict, store_data: bool, store_alerts: bool) -> Dict[str, Any]:
        """Build successful analysis result with store_alerts info"""
        return {
